function [problem, max_level] = sudoku_backtrack_single(problem)
max_level = 1;
...
single = find(problem==0 & len==1,1); % Stores all indices of entries with 1 candidate
empt = find(problem==0 & len==0,1); % Stores all indices of entries with 0 candidate
while isempty(empt) && ~isempty(single)
    problem(single) = cell2mat(C{single}); % Fill in all entries with only 1 candidate
    C = candidates(problem); % Find candidates again with updated matrix
    len = cellfun(@length, C); % Number of candidates for an unfilled entry
    single = find(problem==0 & len==1,1);  % Stores all indices of entries with 1 candidate
    empt = find(problem==0 & len==0,1);  % Stores all indices of entries with 0 candidate
end 
